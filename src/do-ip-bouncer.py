...
parser.add_argument('--fence-ip', dest='FENCE_IP', required=True, default=None, type=str, help='The IP address to fence amongst the cluster.')
...
parser.add_argument('--ip-verify-rate', dest='IP_VERIFY_RATE', default=60, type=int, help='How often for the leader to check that the IP is still assigned. Default: 60')
args = parser.parse_args()


# DO specific stuff
droplet_id = requests.get("http://169.254.169.254/metadata/v1/id").text
manager = digitalocean.Manager()
ip = manager.get_floating_ip(args.FENCE_IP)
...
def leader_changed():
    ...
    while LEADER == args.SELF_ADDR:
        while ip.droplet == None or str(droplet_id) != str(ip.droplet['id']):
            logging.info("attempting to assign ip to this node")
            if not ip.locked:
                ip.assign(droplet_id)
            time.sleep(1)
            ip.load()

        logging.info("ip is assigned to this node")
    
        time.sleep(args.IP_VERIFY_RATE)
